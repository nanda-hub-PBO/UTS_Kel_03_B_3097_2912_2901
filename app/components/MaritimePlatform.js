    ...
    <nav className="navbar">
      <div className="navbar-container">
        ...
        <button 
          type="button" 
          className="nav-brand" 
          onClick={() => handleSelectTab(isAuthenticated ? 'dashboard' : 'company')}
        >
          <Image 
        ...
        <ul className="nav-links">
          {visibleTabs.map((tab) => (
            ...
            >
              ...
              {hoveredTab === tab.id && submenuContent[tab.id] && (
                <NavSubmenu tabId={tab.id} onSelectTab={handleSelectTab} />
              )}
            </li>
          ))}
        </ul>
          ...
          {isAuthenticated ? (
            <>
              ...
              {userRole === 'admin' && (
                <button
                  type="button"
                  className={`btn-admin ${currentTab === 'admin' ? 'active' : ''}`}
            ...
            <>
              <button 
                ...
                onClick={() => {
                  setCompanyScrollTarget('contact')
                  setCompanyScrollNonce(n => n + 1)
                  handleSelectTab('company')
                }}
              >
                ✉ Hubungi Kami
              </button>
